...
class MahasiswaController extends Controller
{
    ...
    public function getMahasiswaByNim(Request $request, $nim)
    {
        ...
        $mahasiswa = Mahasiswa::where('nim', $nim)->with('prodi', 'mahasiswa_matakuliah')->has('mahasiswa_matakuliah')->get()->first();
        if ($mahasiswa == null) {
    ...
    {
        try {
            $mahasiswa = Mahasiswa::where('nim', $request->mhs->nim)->with('prodi', 'mahasiswa_matakuliah')->has('mahasiswa_matakuliah')->get()->first();
            if($mahasiswa == null){
                $mahasiswa = Mahasiswa::where('nim', $request->mhs->nim)->with('prodi')->get()->first();
            }
            return response()->json([
